...
void addCollision(Particle &p)
{
	for (int i = 0; i < 3; i++)
	{
		...
		{
			p.getPos()[i] = size[i] + cpoint[i] - 0.4f;
			p.getVel() *= 0.5f;
		}
	}
}
void createParticles(Particle p[10][10], Force *g, Force *drag)
{
	float gapx = 1.0f;
	float gapz = 1.0f;
	for (int i = 0; i<10; i++)
	{
		for (int j = 0; j < 10; j++)
		{
			p[i][j].translate(glm::vec3(-3.0f + gapx, 5.0f, 0.0f + gapz));
	...
	*/
	...
	while (!glfwWindowShouldClose(app.getWindow()))
	{
		...
		while (accumulator >= dt)
		{
			...
			setVelocitySI(p, dt);
			accumulator -= dt;
			std::cout << p[5][5].getPos().y << std::endl;
		}
